...
Input=(data.sourcepcnp-min(min(data.sourcepcnp)))/(max(max(data.sourcepcnp))-min(min(data.sourcepcnp)))*(0.9-0.1)+0.1
for time=1:10
...
TestActualOutput=(PreTestOut-0.1)*(max(max(data.sourcepcnp))-min(min(data.sourcepcnp)))/(0.9-0.1)+min(min(data.sourcepcnp));
